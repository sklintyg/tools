...
# Denna måste ligga i working directory vid körning. Efter att scriptet körs så läggs resultatet i it_gdpr_dump_result.txt i working directory.
####################
...
#outer left join, on VARDGIVARE_ID, fill in 'NEJ' for missing values
echo "$(join -t $'\t' -a 1 -1 1 -2 1 -e 'NEJ' -o '0,1.2,1.3,2.1' <(echo "$CLEANED_MAIN_DATA") <(echo "$CLEANED_AUGMENT_DATA"))" >> it_gdpr_dump_result.txt
